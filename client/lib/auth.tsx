...
import { api } from "./api";
type AuthContextValue={user:User|null;loading:boolean;login:(email:string,password:string)=>Promise<User>;register:(body:object)=>Promise<User>;google:(credential:string,role?:string)=>Promise<User>;logout:()=>void;refresh:()=>Promise<void>};
...
export function AuthProvider({children}:{children:React.ReactNode}) {
 ...
 const google=async(credential:string,role="supporter")=>save(await api("/auth/google",{method:"POST",body:JSON.stringify({credential,role})}));
 const logout=()=>{localStorage.removeItem("fundora_token");setUser(null);location.href="/"};
 return <AuthContext.Provider value={{user,loading,login,register,google,logout,refresh}}>{children}</AuthContext.Provider>
}
